<template>
  ...
  <v-flex v-else class="text-xs-left">
    <v-layout wrap>
        ...
        <v-card v-if="resource.domain_siblings">
          ...
          <v-card-text>
            <v-flex class="siblings_list">
              <div
                v-for="(domain, index) in domain_siblings_filter_list"
                :key="index"
              >
                {{ domain }}
              </div>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn
                    ...
                    color="grey"
                    v-on="on"
                    @click.stop="copy_content(resource.whois)"
                  >
                    <v-icon>file_copy</v-icon>
                  </v-btn>
                </template>
      ...
      <v-flex
        v-if="resource.undetected_downloaded_samples.length > 0"
        xs10
        lg10
      >
        <v-card>
      ...
      <v-flex v-if="resource.detected_urls.length > 0" xs10 lg10>
        <v-card>
          ...
          <v-card-text>
            <v-flex>
              <v-card
                v-for="(item, index) in resource.detected_urls"
                :key="index"
              >
                <v-card-title>
      ...
      <v-flex v-if="resource.resolutions.length > 0" xs10 lg10>
        <v-card>
          ...
          <v-card-text>
            <v-flex>
              <v-card
                v-for="(item, index) in resource.resolutions"
                :key="index"
              >
                <v-card-title>
