    ...
    <v-layout wrap>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn
                    ...
                    color="grey"
                    v-on="on"
                    @click.stop="copy_content(transform_into_lines(domain_siblings_filter_list))"
                  >
      ...
      <v-flex v-if="resource.https_certificate_date" xs4 lg4 shrink>
        <v-card>
          ...
          <v-divider></v-divider>
          <v-card-text>
            <v-flex class="text-xs-center">{{ get_time(resource.https_certificate_date) }}</v-flex>
          </v-card-text>
        </v-card>
      </v-flex>
      ...
      <v-flex v-if="resource.dns_records_date" xs4 lg4 shrink>
        <v-card>
          ...
          <v-divider></v-divider>
          <v-card-text>
            <v-flex class="text-xs-center">{{ get_time(resource.dns_records_date) }}</v-flex>
          </v-card-text>
        </v-card>
      </v-flex>

      <v-flex v-if="resource.undetected_downloaded_samples.length > 0" xs10 lg10>
        <v-card>
          <v-card-title class="subheading">
            <v-flex>Undetected downloaded samples</v-flex>
            <v-flex>
              <v-chip>{{ resource.undetected_downloaded_samples.length }}</v-chip>
            </v-flex>
            <v-spacer></v-spacer>
            <v-flex>
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn
                    ...
                    color="grey"
                    v-on="on"
                    @click.stop="copy_content(resource.undetected_downloaded_samples.map(elem => elem.sha256).join('\n'))"
                  >
                    <v-icon>file_copy</v-icon>
                  </v-btn>
                </template>
                ...
                <v-card-title>
                  ...
                  <v-flex>
                    ...
                    >
                      ...
                      >{{ get_percentage(item.positives, item.total) }}&nbsp;%</span>
                    </v-progress-circular>
                  </v-flex>
                </v-card-title>
              </v-card>
      ...
      <v-flex v-if="resource.detected_downloaded_samples.length > 0" xs10 lg10>
        <v-card>
          <v-card-title class="subheading">
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn
                    ...
                    color="grey"
                    v-on="on"
                    @click.stop="copy_content(resource.detected_downloaded_samples.map(elem => elem.sha256).join('\n'))"
                  >
                    <v-icon>file_copy</v-icon>
                  </v-btn>
                </template>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn
                    ...
                    color="grey"
                    v-on="on"
                    @click.stop="copy_content(resource.detected_urls.map(elem => elem.url).join('\n'))"
                  >
                    <v-icon>file_copy</v-icon>
                  </v-btn>
                </template>
